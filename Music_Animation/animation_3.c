...
void boid_algo_init_calc(uint16_t curr_boid)
{
    ...
    if (difference_from_overall_color > 0) curr_flock[curr_boid].hue -= 5;
    else curr_flock[curr_boid].hue += 5;

    // overall_mood moves to 0.5 saturation value
    diff_from_overall_saturation = 0.5 - curr_flock[curr_boid].sat; // float
    if (diff_from_overall_saturation > 0) curr_flock[curr_boid].sat -= 0.01;
    else curr_flock[curr_boid].sat += 0.01;
    ...
    while (1)
    {
        ...
        else if (strcmp(cmd, "numberPredators") == 0)
        {
            ...
            if (arg1 != NULL)
            {
                for (uint16_t current_boid = 0; current_boid < curr_N_boids; current_boid++)
                {
                    fillCircle(fix2int15(boid_flock[current_boid].x), fix2int15(boid_flock[current_boid].y), 2, BLACK);
                }
...
static PT_THREAD(protothread_FFT(struct pt *pt))
{
    ...
    while (1)
    {
        ...
        if (abs(percent_diff) > percent_diff_threshold) { // These are two different situations
